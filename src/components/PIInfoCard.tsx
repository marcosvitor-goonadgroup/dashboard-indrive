...
const PIInfoCard = ({ numeroPi, campaignData = [] }: PIInfoCardProps) => {
  ...
  const veiculacaoPorVeiculoTipo = useMemo(() => {
    ...
    const piGrouped = new Map<string, {
      ...
      tipoDeCompra: string;
      previsto: number;
      quantidade: number; // volume contratado
    }>();
    ...
    piGrouped.forEach((piData) => {
      ...
      // Se não encontrou, tenta apenas pelo tipo de compra (caso o veiculo seja "Intersticial" ou formato)
      if (!match) {
              ...
              {veiculacaoPorVeiculoTipo.map((item, index) => {
                ...
                return (
                  <div key={index} className="border-l-4 border-blue-500 pl-3 py-2 bg-gray-50 rounded">
                    ...
                    {/* Barra de pacing de volume (CPM = impressões / CPC = cliques) */}
                    {(isCPM || isCPC) && item.quantidade > 0 && (
                      <div>
                        <div className="flex justify-between items-center mb-1">
